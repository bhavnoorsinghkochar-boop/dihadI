...
> = ({
  ...
  /* Google Workspace Authorization */ const handleLinkGoogle = async () => { setIsLinkingGoogle(true); try { await requestGmailAccessToken(); playSound('success'); setInlineSuccess('✓ Google Workspace Gmail linked successfully!'); handleSendOtp(true); } catch (err: any) { console.warn(err); setInlineError(err?.message || 'Google authorization could not be completed.'); } finally { setIsLinkingGoogle(false); } }; if (!isOpen) return null; return ( <div id="gmail-otp-modal-backdrop" className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4 animate-in fade-in" > <div id="gmail-otp-modal-container" className="bg-white rounded-3xl max-w-md w-full overflow-hidden shadow-2xl border border-slate-200 flex flex-col transition-all" > {/* Modal Header */} <div className="bg-gradient-to-r from-slate-900 via-slate-800 to-amber-950 text-white p-4 sm:p-5 flex items-center justify-between shrink-0"> <div className="flex items-center gap-3"> <div className="w-10 h-10 rounded-2xl bg-amber-500/20 border border-amber-400/30 flex items-center justify-center text-amber-400"> <Mail className="w-5 h-5" /> </div> <div> <h3 className="text-base font-black text-white flex items-center gap-1.5"> <span>{title}</span> <span className="px-2 py-0.5 bg-amber-500/20 text-amber-300 text-[10px] font-bold rounded-full border border-amber-400/20 uppercase"> Google Workspace </span> </h3> <p className="text-xs text-slate-300 line-clamp-1">{subtitle}</p> </div> </div> <button id="close-gmail-modal-btn" onClick={onClose} className="w-8 h-8 rounded-full bg-slate-800 hover:bg-slate-700 text-slate-300 flex items-center justify-center transition cursor-pointer" aria-label="Close modal" > <X className="w-4 h-4" /> </button> </div> {/* Modal Body */} <div className="p-5 sm:p-6 space-y-4 overflow-y-auto"> {/* STEP 1: Email Input Form */} <div className="space-y-3"> <div className="flex items-center justify-between"> <label htmlFor="gmail-email-input" className="text-xs font-bold text-slate-700 flex items-center gap-1.5" > <Mail className="w-3.5 h-3.5 text-amber-600" /> <span>Gmail Address</span> </label> {step === 'verify_otp' && ( <button type="button" onClick={() => { setStep('input_email'); setInlineError(null); setInlineSuccess(null); }} className="text-[11px] font-bold text-amber-600 hover:text-amber-800 transition cursor-pointer" > Change Email </button> )} </div> <div className="flex gap-2"> <div className="relative flex-1"> <input id="gmail-email-input" type="email" value={email} disabled={isSendingOtp || isVerifyingOtp || step === 'verify_otp'} onChange={(e) => { setEmail(e.target.value); if (emailValidationError) setEmailValidationError(null); if (inlineError) setInlineError(null); }} placeholder="yourname@gmail.com" className={`w-full bg-slate-50 border ${ emailValidationError ? 'border-amber-400 focus:border-amber-600 focus:ring-amber-200' : 'border-slate-300 focus:border-amber-600 focus:ring-amber-100' } rounded-xl px-3.5 py-2.5 text-xs sm:text-sm font-semibold text-slate-900 focus:outline-none focus:ring-2 disabled:bg-slate-100 disabled:text-slate-500 transition`} /> </div> {step === 'input_email' && ( <button id="send-otp-btn" type="button" onClick={() => handleSendOtp(false)} disabled={isSendingOtp} className="px-4 py-2.5 bg-amber-600 hover:bg-amber-700 active:scale-[0.98] disabled:opacity-50 text-white rounded-xl text-xs sm:text-sm font-bold shrink-0 transition flex items-center justify-center gap-1.5 shadow-sm cursor-pointer min-w-[100px]" > {isSendingOtp ? ( <> <Loader2 className="w-4 h-4 animate-spin" /> <span>Sending...</span> </> ) : ( <> <span>Send OTP</span> <ArrowRight className="w-3.5 h-3.5" /> </> )} </button> )} </div> {emailValidationError && ( <p className="text-[11px] font-semibold text-amber-600 flex items-center gap-1"> <AlertCircle className="w-3.5 h-3.5 shrink-0" /> <span>{emailValidationError}</span> </p> )} </div> {/* STEP 2: 6-Digit OTP Input & Countdown */} {step === 'verify_otp' && ( <div className="space-y-3 pt-2 border-t border-slate-100 animate-in fade-in slide-in-from-top-2"> <div className="flex items-center justify-between"> <label htmlFor="gmail-otp-input" className="text-xs font-bold text-slate-800 flex items-center gap-1.5" > <KeyRound className="w-3.5 h-3.5 text-amber-600" /> <span>Enter 6-Digit OTP</span> </label> <div className="flex items-center gap-1 text-[11px] text-slate-500 font-medium"> <Clock className="w-3 h-3 text-slate-400" /> <span>Valid for 10 mins</span> </div> </div> <div className="relative"> <input ref={otpInputRef} id="gmail-otp-input" type="text" inputMode="numeric" pattern="[0-9]*" maxLength={6} value={otp} onChange={(e) => { const clean = e.target.value.replace(/[^0-9]/g, ''); setOtp(clean); if (inlineError) setInlineError(null); }} onKeyDown={(e) => { if (e.key === 'Enter' && otp.length === 6) { handleVerifyOtp(); } }} placeholder="• • • • • •" className="w-full text-center tracking-[0.45em] font-mono text-2xl font-black bg-slate-50 border border-slate-300 rounded-2xl py-3 text-slate-900 focus:outline-none focus:border-amber-600 focus:ring-2 focus:ring-amber-100 focus:bg-white transition shadow-inner" /> </div> {/* Dev/Preview 1-Tap Helper Bar */} {generatedOtp && ( <div className="bg-amber-50/80 border border-amber-200/80 rounded-2xl p-2.5 flex items-center justify-between gap-2"> <div className="flex items-center gap-2"> <span className="w-6 h-6 rounded-lg bg-amber-600 text-white font-mono font-bold text-[10px] flex items-center justify-center"> OTP </span> <span className="text-xs font-mono font-black text-amber-900">{generatedOtp}</span> </div> <div className="flex items-center gap-1"> <button type="button" onClick={handleCopyCode} className="text-[11px] font-bold text-amber-700 hover:text-amber-900 px-2 py-1 bg-white rounded-lg border border-amber-200 flex items-center gap-1 cursor-pointer transition" > {isCopied ? <Check className="w-3 h-3 text-amber-600" /> : <Copy className="w-3 h-3" />} <span>{isCopied ? 'Copied' : 'Copy'}</span> </button> <button type="button" onClick={handle1TapAutofill} className="text-[11px] font-black text-white bg-amber-600 hover:bg-amber-700 px-2.5 py-1 rounded-lg flex items-center gap-1 cursor-pointer transition shadow-xs" > <Sparkles className="w-3 h-3 text-amber-300" /> <span>Autofill</span> </button> </div> </div> )} {/* Verify Action Button */} <button id="verify-otp-btn" type="button" onClick={handleVerifyOtp} disabled={isVerifyingOtp || otp.length < 6} className="w-full bg-amber-600 hover:bg-amber-700 active:scale-[0.99] disabled:opacity-40 text-white font-black py-3.5 rounded-2xl shadow-md text-xs sm:text-sm transition flex items-center justify-center gap-2 cursor-pointer mt-1" > {isVerifyingOtp ? ( <> <Loader2 className="w-4 h-4 animate-spin" /> <span>Verifying OTP...</span> </> ) : ( <> <CheckCircle2 className="w-4 h-4 text-amber-300" /> <span>Verify & Continue</span> </> )} </button> {/* 60-Second Countdown and Resend Controls */} <div className="flex items-center justify-between text-xs text-slate-600 px-1 pt-1"> <span>Didn't receive the OTP?</span> {countdown > 0 ? ( <span className="font-mono text-slate-400 font-semibold text-[11px] flex items-center gap-1"> <Clock className="w-3 h-3" /> <span>Resend in {countdown}s</span> </span> ) : ( <button id="resend-otp-btn" type="button" onClick={() => handleSendOtp(true)} disabled={isSendingOtp} className="font-bold text-amber-600 hover:text-amber-800 disabled:opacity-50 flex items-center gap-1 cursor-pointer transition" > {isSendingOtp ? ( <> <Loader2 className="w-3 h-3 animate-spin" /> <span>Sending...</span> </> ) : ( <> <RefreshCw className="w-3 h-3" /> <span>Resend OTP</span> </> )} </button> )} </div> </div> )} {/* Inline Success Message Banner */} {inlineSuccess && ( <div id="gmail-otp-success-msg" className="bg-amber-50 border border-amber-200 text-amber-900 text-xs p-3 rounded-2xl flex items-start gap-2 animate-in fade-in" > <CheckCircle2 className="w-4 h-4 text-amber-600 shrink-0 mt-0.5" /> <p className="font-medium leading-relaxed">{inlineSuccess}</p> </div> )} {/* Inline Error Message Banner */} {inlineError && ( <div id="gmail-otp-error-msg" className="bg-amber-50 border border-amber-200 text-amber-900 text-xs p-3 rounded-2xl flex items-start gap-2 animate-in fade-in" > <AlertCircle className="w-4 h-4 text-amber-600 shrink-0 mt-0.5" /> <p className="font-medium leading-relaxed">{inlineError}</p> </div> )} {/* Gmail Quick Access Shortcuts */} <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-[11px]"> <a href="https://mail.google.com" target="_blank" rel="noopener noreferrer" className="text-slate-600 hover:text-amber-600 font-semibold flex items-center gap-1 transition" > <Mail className="w-3.5 h-3.5 text-red-500" /> <span>Open Gmail Inbox</span> <ExternalLink className="w-3 h-3 opacity-60" /> </a> <button type="button" onClick={handleLinkGoogle} disabled={isLinkingGoogle} className="text-amber-600 hover:text-amber-800 font-bold flex items-center gap-1 cursor-pointer transition" > {isLinkingGoogle ? ( <> <Loader2 className="w-3 h-3 animate-spin" /> <span>Linking Google...</span> </> ) : ( <> <Sparkles className="w-3 h-3 text-amber-500" /> <span>Google Workspace OAuth</span> </> )} </button> </div> </div> </div> </div> );
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-500">
              Fast 6-digit OTP verification via Google Workspace
            </p>{" "}
          </div>{" "}
        </div>{" "}
      </div>{" "}
