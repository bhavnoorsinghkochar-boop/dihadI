    ...
    <div className="w-full max-w-5xl mx-auto p-4 sm:p-6 lg:p-8 flex flex-col items-center justify-center space-y-8 select-none">
      ...
      <div className="w-full grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 space-y-1.5 text-[11px] text-slate-600">
              <div className="flex items-center gap-1.5">
                ...
                <span>
                  {currentWorker ? (
                    <strong className="text-amber-700">{currentWorker.name} ({currentWorker.primaryTrade})</strong>
                  ) : (
        ...
        <div 
          ...
          className="group bg-white rounded-3xl p-6 border-2 border-slate-200 hover:border-amber-600 hover:shadow-xl transition-all duration-200 cursor-pointer flex flex-col justify-between space-y-5"
        >
          <div className="space-y-4">
            <div className="w-14 h-14 bg-amber-50 rounded-2xl flex items-center justify-center text-amber-600 border border-amber-200 group-hover:scale-105 transition">
            ...
            <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 space-y-1.5 text-[11px] text-slate-600">
              <div className="flex items-center gap-1.5">
                ...
                <span>
                  {currentCustomer ? (
                    <strong className="text-amber-700">{currentCustomer.name} ({currentCustomer.area})</strong>
                  ) : (
                    <span>{getT(currentLanguage, 'feature_id_pass_login')}</span>
                  )}
                </span>
          ...
          <button className="w-full bg-amber-600 hover:bg-amber-700 text-white py-3 rounded-2xl font-bold text-xs transition flex items-center justify-center gap-2 shadow-sm shadow-amber-200">
        ...
        >
          <div className="space-y-4">
            <div className="w-14 h-14 bg-slate-800 rounded-2xl flex items-center justify-center text-amber-400 border border-slate-700 group-hover:scale-105 transition">
            ...
            <div>
              <div className="flex items-center justify-between">
                ...
                <span className="text-[10px] font-bold px-2 py-0.5 rounded bg-amber-900/60 text-amber-300 border border-amber-700/50">
            ...
            <div className="bg-slate-800/80 p-3 rounded-xl border border-slate-700 space-y-1.5 text-[11px] text-slate-300">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span>
                  {currentAdmin ? <strong className="text-amber-300">{currentAdmin.name}</strong> : getT(currentLanguage, 'feature_admin_protected')}
                </span>
              </div>
            </div>
          </div>

          <button className="w-full bg-slate-800 group-hover:bg-amber-600 text-white py-3 rounded-2xl font-bold text-xs transition flex items-center justify-center gap-2 shadow-sm">
      ...
      <div className="w-full flex flex-col sm:flex-row items-center justify-between text-xs text-slate-500 bg-slate-200/60 px-4 py-2.5 rounded-xl border border-slate-300/60 gap-2">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={resetToZero}
            className="px-2.5 py-1 bg-white hover:bg-amber-50 text-amber-700 border border-slate-300 rounded-lg text-[11px] font-bold transition flex items-center gap-1"
