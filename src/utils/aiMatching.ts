import { WorkerProfile, TradeType, HyperlocalMatchResult, MultiChannelAlertPayload, Job, Language } from '../types';
...
export function matchHyperlocalWorkers(
  ...
  const eligibleWorkers = workers.filter((worker) => {
    ...
    const distanceKm = calculateDistanceKm(criteria.lat, criteria.lng, workerLat, workerLng);
  ...
  const results: HyperlocalMatchResult[] = eligibleWorkers.map((worker) => {
    ...
    if (worker.primaryTrade.toLowerCase() === criteria.trade.toLowerCase()) {
      ...
      skillScore = 35;
    } else if (worker.secondaryTrades && worker.secondaryTrades.some(t => t.toLowerCase() === criteria.trade.toLowerCase())) {
    ...
    let distanceScore = 0;
    if (isWithin10Km) {
      distanceScore = Math.max(10, Math.round(40 - (distanceKm / maxDistance) * 30));
    }
    ...
    // Overall Score (0 - 100)
    let totalScore = Math.min(100, Math.round(skillScore + distanceScore + availabilityScore + trustScore));
    ...
    if (distanceKm <= 3.0) {
      reasonParts.push(`Hyperlocal (${distanceKm} km, ~${etaMinutes} mins away)`);
    ...
    if (skillMatch === 'exact_primary') {
      reasonParts.push(`Master ${worker.primaryTrade} (${worker.experienceYears} yrs exp)`);
