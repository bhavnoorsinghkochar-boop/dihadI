...
import { MessageCircle, Search, Clock, Send, ShieldAlert, Phone } from 'lucide-react';
...
export const AdminSupportChatHub: React.FC = () => {
  ...
  return (
    <div className="bg-slate-800/50 border border-slate-700 rounded-2xl overflow-hidden h-[600px] flex">
      {/* Sidebar: Conversation List */}
      <div className="w-1/3 border-r border-slate-700 flex flex-col bg-slate-900/50">
        <div className="p-4 border-b border-slate-700">
          <h3 className="font-bold text-white text-sm flex items-center gap-2">
            <MessageCircle className="w-4 h-4 text-amber-400" />
            Support Hub ({conversations.length})
          </h3>
        </div>
      ...
      <div className="w-2/3 flex flex-col bg-slate-950">
        {selectedConversation && selectedConvoData ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-slate-500 gap-4">
            ...
            <p className="text-sm font-medium">Select a conversation to view and reply.</p>
          </div>
        )}
      </div>
    </div>
