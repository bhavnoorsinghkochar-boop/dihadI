...
export const PitchDeckViewer: React.FC = () => {
  ...
  const slides = [
    ...
    {
      title: 'Our Team',
      subtitle: 'The Founders & Visionaries behind Dihadi.co',
      team: [
        ...
        { name: 'Bhavnoor Singh', role: 'Co-Founder & Tech Architect', image: 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=200&auto=format&fit=crop&q=80' }
      ],
      type: 'team'
    },
    {
    ...
    },
    {
      ...
      items: [
        { title: 'Long Exploitative Hours', desc: 'Workers frequently work 12–16 hours per day with irregular rest and no safety nets.' },
        { title: 'Hazardous Environments', desc: 'Workplaces often lack basic safety standards, with high risk of unpaid injury and accidents.' },
        { title: 'Unfair Wages & Cuts', desc: 'Middlemen and labor contractors siphon 30–50% of the wage, leaving workers below living needs.' },
        { title: 'Unorganized Market', desc: 'No digital reputation, zero banking access, and no transparent pricing mechanism.' }
      ],
      type: 'grid'
    },
    {
      title: 'What is the Solution?',
      subtitle: 'OUR APP: Dihadi.co Unified Marketplace',
      highlights: [
        { title: 'Direct Connection', desc: 'Dihadi.co connects workers directly with employers without an exploitative middleman.' },
    ...
    },
    {
      ...
      pillars: [
        { title: 'Customer App', desc: 'Find, book, and pay verified daily wage workers with transparent upfront pricing.' },
        { title: 'Worker App', desc: 'Build a verified digital profile, get instant job requests, and track daily earnings.' },
        { title: 'Admin Dashboard', desc: 'Verify worker Aadhaar/skills, monitor active jobs live, resolve disputes and power payouts.' }
      ],
      type: 'pillars'
    },
    {
      ...
      revenueStreams: [
        ...
        { name: 'Verified Badge Fast-Track', desc: 'Expedited physical document inspection & trade skill accreditation.' },
        { name: 'Local Business Partnerships', desc: 'Hardware stores, paint distributors, and material suppliers offering tool discounts to workers.' }
      ],
      type: 'business'
    },
    {
      title: 'Market Research & White Space',
      subtitle: 'Dihadi.co vs Urban Company vs Digital Labour Chowk',
      comparison: [
        ...
        { feature: 'Payment Flow', uc: 'In-app payment', dlc: 'Handled offline between parties (disputes)', dihadi: 'Secure In-App Escrow & Instant UPI Payout' },
        { feature: 'Consumer Experience', uc: 'Consumer app only for luxury home care', dlc: 'Mainly Contractor/B2B facing', dihadi: 'Consumer-First Mobile App for Raw Daily Labor' }
      ],
      type: 'market'
    },
    {
      title: 'SWOT Analysis',
      subtitle: 'Strategic Overview of Dihadi.co',
      swot: {
        strengths: [
    ...
    <div className="max-w-4xl mx-auto bg-white rounded-3xl border border-slate-200 shadow-xl overflow-hidden flex flex-col min-h-[600px] select-none">
      ...
      <div className="h-14 bg-slate-900 text-white px-6 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-bold tracking-tight">Dihadi.co Pitch Deck</span>
        </div>
      ...
      <div className="flex-1 p-8 sm:p-10 bg-[#FAFAFA] flex flex-col justify-center">
        {slide.type === 'cover' && (
          <div className="text-center space-y-6 max-w-2xl mx-auto">
            ...
            <div className="pt-4 flex flex-wrap justify-center gap-2">
              {slide.sdg?.map((s) => (
                <span key={s} className="px-3 py-1.5 bg-white border border-slate-200 shadow-xs rounded-xl text-xs font-bold text-slate-800">
                  🌱 {s}
                </span>
              ))}
            </div>
        ...
        {slide.type === 'team' && (
          <div className="space-y-6">
            <div className="text-center">
              <h2 className="text-3xl font-black text-slate-900">{slide.title}</h2>
              <p className="text-sm text-slate-500 mt-1">{slide.subtitle}</p>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 pt-4">
              {slide.team?.map((member) => (
                <div key={member.name} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm text-center space-y-3">
                  <img
                    src={member.image}
                    alt={member.name}
          ...
          <div className="space-y-6 max-w-2xl mx-auto">
            ...
            <div className="space-y-3 pt-2">
              {slide.points?.map((p, idx) => (
                <div key={idx} className="bg-white p-4 rounded-2xl border border-slate-200 shadow-xs flex items-start gap-3">
                  ...
                  <p className="text-sm text-slate-700 leading-relaxed font-medium">{p}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
              {slide.highlights?.map((h) => (
                <div key={h.title} className="bg-white p-5 rounded-2xl border border-amber-200 shadow-xs space-y-1.5">
        ...
        {slide.type === 'market' && (
          <div className="space-y-5">
            <div>
              <h2 className="text-2xl font-black text-slate-900">{slide.title}</h2>
              ...
              <table className="w-full text-left text-xs bg-white rounded-2xl border border-slate-200 overflow-hidden">
                <thead className="bg-slate-100 text-slate-700 font-bold border-b border-slate-200">
                  <tr>
                    ...
                    <th className="p-3 text-amber-700 bg-amber-50">Dihadi.co (Winner)</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {slide.comparison?.map((row) => (
                    <tr key={row.feature}>
                      ...
                      <td className="p-3 font-semibold text-slate-900 bg-amber-50/50">{row.dihadi}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div className="bg-amber-50/70 border border-amber-200 p-3.5 rounded-2xl space-y-1">
                <h3 className="font-extrabold text-xs text-amber-900 uppercase">WEAKNESSES</h3>
