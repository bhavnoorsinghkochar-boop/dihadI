...
interface CustomerSubscriptionModalProps {
  ...
  onSubscribe: (method: 'UPI' | 'CARD' | 'NET_BANKING') => Promise<{ success: boolean; message: string }> | { success: boolean; message: string };
}
...
export const CustomerSubscriptionModal: React.FC<CustomerSubscriptionModalProps> = ({
  ...
  const expiryDateFormatted = customer.premiumCustomerExpiresAt 
    ? new Date(customer.premiumCustomerExpiresAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })
    : null;
    ...
    <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-3 sm:p-4 animate-in fade-in duration-200">
      <div className="bg-white rounded-3xl max-w-lg w-full overflow-hidden shadow-2xl border border-amber-300 relative max-h-[92vh] flex flex-col">
          ...
          {showSuccessState ? (
            <div className="text-center py-6 space-y-4 animate-in zoom-in-95">
              ...
              <div className="space-y-1">
                <h3 className="text-xl font-black text-slate-900">Welcome to Dihadi Gold Club!</h3>
                <p className="text-xs text-amber-700 font-bold">
                  ₹{PRICE.toLocaleString('en-IN')} Subscription Initiated & Received in Admin Account
                </p>
                <p className="text-xs text-slate-600 pt-1">
                  When you hire workers, the <strong>Admin will automatically disburse worker wages directly to their wallet</strong> with ₹0 platform surcharge for the next 30 days!
                </p>
              </div>
              ...
              <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 text-left space-y-2 text-xs">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-black text-amber-800">1 Month (30 Days)</span>
                </div>
                <div className="flex items-center justify-between">
              ...
              >
                Hire Workers with 1 Month Free Service
              </button>
            </div>
          ) : showUpiQr ? (
            /* UPI QR Modal Screen */
            <div className="space-y-4 text-center animate-in fade-in">
              <div className="space-y-1">
                <h3 className="text-lg font-black text-slate-900">Scan UPI QR to Pay ₹{PRICE.toLocaleString('en-IN')}</h3>
                <p className="text-xs text-slate-500">
                  Payment routes directly to Dihadi Admin Escrow Account
                </p>
              </div>
              ...
              <div className="bg-white p-4 rounded-2xl max-w-[200px] mx-auto shadow-lg border-2 border-amber-400 flex flex-col items-center">
                <img
                  src={`https://api.qrserver.com/v1/create-qr-code/?size=180x180&data=${encodeURIComponent(
                    `upi://pay?pa=dihadi.admin@okhdfcbank&pn=Dihadi%20Admin%20Treasury&am=${PRICE}&cu=INR&tn=Customer%20Gold%2015000%20Subscription`
                  )}`}
                  alt="UPI QR"
                  className="w-36 h-36 rounded-lg"
                />
                <span className="text-[10px] text-slate-600 font-mono font-bold mt-2">
                  dihadi.admin@okhdfcbank
                </span>
              </div>
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-2xl p-3 text-xs text-left space-y-1.5">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-bold text-amber-700">Dihadi Admin Treasury</span>
                </div>
            ...
            <>
              ...
              <div className="bg-gradient-to-br from-amber-500/10 via-amber-400/20 to-amber-500/10 border-2 border-amber-400 rounded-3xl p-5 relative overflow-hidden">
                <div className="flex items-baseline justify-between">
                  <div>
                    <span className="text-[10px] font-black text-amber-800 uppercase tracking-wider block">
                      Customer Gold Subscription
                    </span>
                    <div className="flex items-baseline gap-1.5 mt-0.5">
                      <span className="text-3xl font-black text-slate-950 font-mono">₹{PRICE.toLocaleString('en-IN')}</span>
                      <span className="text-xs text-slate-600 font-medium">/ 1 Month Free Service</span>
                    </div>
                  </div>
                ...
                <div className="mt-3 pt-3 border-t border-amber-300/80 flex items-center gap-2 text-xs text-amber-950 font-bold">
                  ...
                  <span>Money credited directly to Admin Account • Admin auto-pays worker wages to their wallet!</span>
                </div>
              </div>
              ...
              <div className="space-y-2.5">
                <h4 className="text-xs font-black text-slate-800 uppercase tracking-wider">
                  Exclusive Gold Member Privileges:
                </h4>
                ...
                <div className="grid grid-cols-1 gap-2 text-xs">
                  <div className="flex items-start gap-2.5 bg-amber-50/70 p-2.5 rounded-xl border border-amber-200">
                    ...
                    <div>
                      <strong className="text-slate-900">Admin Auto-Pays Worker Wallets:</strong>
                      <p className="text-slate-600 text-[11px]">
                        When you hire a worker, the platform Admin automatically transfers the worker's earnings straight to their wallet from the admin treasury.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-2.5 bg-slate-50 p-2.5 rounded-xl border border-slate-200">
                    ...
                    <div>
                      <strong className="text-slate-900">1 Month Free Service (Zero Booking Fees):</strong>
                      <p className="text-slate-500 text-[11px]">
                        Post unlimited jobs and hire masons, plumbers, electricians with ₹0 platform surcharge.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-2.5 bg-slate-50 p-2.5 rounded-xl border border-slate-200">
                    ...
                    <div>
                      ...
                      <p className="text-slate-500 text-[11px]">
                        Your job alert is highlighted at the top of nearby worker feeds with priority sirens.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-2.5 bg-slate-50 p-2.5 rounded-xl border border-slate-200">
                    ...
                    <div>
                      ...
                      <p className="text-slate-500 text-[11px]">
                        Download official verified background and Aadhaar KYC performance PDF dossiers for free.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-2.5 bg-slate-50 p-2.5 rounded-xl border border-slate-200">
                    ...
                    <div>
                      <strong className="text-slate-900">Dedicated Priority Helpline:</strong>
                      <p className="text-slate-500 text-[11px]">
                        Direct phone line to our emergency supervisor for quick worker replacement or dispute help.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div
                    ...
                    className={`p-3 rounded-2xl border cursor-pointer transition flex items-center gap-2.5 ${
                      selectedMethod === 'UPI'
                        ? 'bg-amber-50 border-amber-500 text-slate-950 font-bold'
                        : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'
                    }`}
                  >
                    <QrCode className="w-4 h-4 text-amber-600 shrink-0" />
                    <div>
                      <div className="font-black text-[11px]">UPI / QR Code</div>
                      <div className="text-[9px] text-slate-400">GPay, PhonePe, Paytm</div>
                    </div>
                  </div>
              ...
              <div className="pt-2">
                ...
                >
                  ...
                  <span>
                    {isProcessing ? 'Activating...' : `Activate 1 Month Gold Plan for ₹${PRICE.toLocaleString('en-IN')}`}
                  </span>
        ...
        <div className="bg-slate-50 p-3 text-center border-t border-slate-200 text-[10px] text-slate-500 shrink-0">
          🛡️ Safe & Secure Payment • Payment deposited into Admin Account • Auto-Wallet Payouts Enabled
        </div>
      </div>
    </div>
  );
