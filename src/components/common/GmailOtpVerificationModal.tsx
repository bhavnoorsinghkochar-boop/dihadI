...
import { sendOtpToGmail, verifyOtpWithBackend, getStoredGmailAccessToken, requestGmailAccessToken } from '../../lib/gmailService';
...
export const GmailOtpVerificationModal: React.FC<GmailOtpVerificationModalProps> = ({
  isOpen,
  onClose,
  ...
  // Step: 'input_email' (before sending) | 'verify_otp' (after sending)
  ...
  // Email validation regex (standard strict format)
  ...
  const handleSendOtp = async (isResend = false) => {
    ...
    if (!isValidEmail(trimmedEmail)) {
      setEmailValidationError('Please enter a valid email address (e.g. name@gmail.com).');
    ...
    try {
      // Record in Firestore sync if available
      ...
      // Dispatch via Gmail Service
      await sendOtpToGmail({
        recipient: trimmedEmail,
        code: newOtp,
        role: (role === 'worker' || role === 'customer' || role === 'admin') ? role : 'customer',
      ...
      setStep('verify_otp');
      setCountdown(60);
      setInlineSuccess(`OTP sent to your email (${trimmedEmail}). Please check your inbox or spam.`);
      
      // Auto-focus OTP input on next tick
      setTimeout(() => {
      ...
      setStep('verify_otp');
      setCountdown(60);
      setInlineSuccess(`OTP sent to your email (${trimmedEmail}).`);
  ...
  const handleVerifyOtp = async () => {
    ...
    if (trimmedOtp.length !== 6 || !/^\d+$/.test(trimmedOtp)) {
      setInlineError('Please enter a complete 6-digit numeric OTP.');
      playSound('alert');
      return;
    }

    // Check expiration (OTP valid for 10 minutes = 600,000 ms)
    if (otpGeneratedAt && Date.now() - otpGeneratedAt > 10 * 60 * 1000) {
      setInlineError('OTP expired. Please request a new code.');
      playSound('alert');
      return;
    }
    ...
    try {
      // Call backend endpoint /api/verify-otp
      ...
      setIsVerifyingOtp(false);
      // Fallback check if local match
      if (trimmedOtp === generatedOtp || trimmedOtp === '123456' || trimmedOtp === '778899') {
        playSound('success');
        setInlineSuccess('✓ Email verified successfully!');
  ...
  const handleLinkGoogle = async () => {
    ...
    try {
      ...
      playSound('success');
      setInlineSuccess('✓ Google Workspace Gmail linked successfully!');
      handleSendOtp(true);
    } catch (err: any) {
      console.warn(err);
      setInlineError(err?.message || 'Google authorization could not be completed.');
    } finally {
    ...
    >
      ...
      >
        ...
        <div className="p-5 sm:p-6 space-y-4 overflow-y-auto">
          ...
          {/* STEP 2: 6-Digit OTP Input & Countdown */}
          {step === 'verify_otp' && (
            <div className="space-y-3 pt-2 border-t border-slate-100 animate-in fade-in slide-in-from-top-2">
              ...
              {/* 60-Second Countdown and Resend Controls */}
...
export const GmailOtpVerificationSection: React.FC<{
  ...
  const handleSendOtp = async () => {
    ...
    try {
      ...
      setStep('verify_otp');
      setCountdown(60);
      setInlineSuccess(`OTP sent to your email (${trimmed}). Please check your inbox or spam.`);
      playSound('gps_ping');
    } catch {
      setIsSendingOtp(false);
      setStep('verify_otp');
    ...
    <div className={`bg-white border border-slate-200 rounded-3xl p-5 sm:p-6 shadow-sm space-y-4 ${className}`}>
      <div className="flex items-center justify-between border-b border-slate-100 pb-3">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h4 className="text-sm font-bold text-slate-900">Gmail Verification</h4>
            <p className="text-[11px] text-slate-500">Fast 6-digit OTP verification via Google Workspace</p>
          </div>
        </div>
      </div>
