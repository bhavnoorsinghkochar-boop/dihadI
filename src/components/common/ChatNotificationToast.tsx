...
import { MessageSquare, X, ArrowRight, CheckCheck, User, Briefcase, Sparkles, Send } from 'lucide-react';
...
const SingleChatToast: React.FC<SingleToastProps> = ({ item, onDismiss, onOpenChat }) => {
  const [progress, setProgress] = useState(100);
  const [isPaused, setIsPaused] = useState(false);

  const durationMs = 6500; // 6.5 seconds auto dismiss
  const intervalMs = 50;
