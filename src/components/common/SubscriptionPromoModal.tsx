...
export const SubscriptionPromoModal: React.FC<SubscriptionPromoModalProps> = ({
  ...
  const [showActivatedSuccess, setShowActivatedSuccess] = useState<boolean>(false);
  ...
  const handleActivateCustomerGold = async () => {
    ...
    playSound('click');
    try {
      const res = subscribeCustomerPremium(currentCustomer?.id || 'cust-1', 'UPI');
      if (res.success) {
        setShowActivatedSuccess(true);
        setSuccessMessage('₹15,000 Customer Gold Subscription Activated! Money credited to Admin Account. Worker wages will now be automatically disbursed by Admin directly into worker wallets!');
        playSound('success');
      }
    } finally {
  ...
  const handleActivateWorkerVip = async () => {
    ...
    try {
      ...
      if (res.success) {
        setShowActivatedSuccess(true);
        setSuccessMessage('Worker VIP Pass & Instant Aadhaar Verification Activated! You are now Govt. Aadhaar Verified with 6 Zero-Commission Jobs and top radar priority!');
        playSound('success');
      }
    } finally {
      setIsProcessing(false);
      ...
      <div className="bg-gradient-to-b from-slate-900 via-slate-900 to-slate-950 text-white rounded-3xl max-w-xl w-full overflow-hidden shadow-2xl border-2 border-amber-400/60 relative flex flex-col max-h-[92vh]">
        ...
        {allowRoleSwitch ? (
          <div className="p-4 bg-slate-900/60 border-b border-slate-800/80 flex gap-2">
            ...
            >
              ...
              <span>Customer Gold Plan (₹15,000)</span>
            </button>
            ...
            >
              ...
              <span>Worker VIP & Aadhaar (₹2,000)</span>
            </button>
          </div>
        ) : (
          <div className="px-5 py-3 bg-slate-900/80 border-b border-slate-800/80 flex items-center justify-between">
            <div className="flex items-center gap-2">
              {activeTab === 'customer' ? (
                <>
                  ...
                  <div>
                    <h4 className="text-xs font-black text-white">Customer Exclusive Membership</h4>
                    <p className="text-[10px] text-amber-400">1 Month Free Service & Admin Automated Worker Payouts</p>
                  </div>
                </>
              ) : (
                <>
            ...
            <span className="text-[10px] font-mono uppercase bg-slate-800 text-slate-300 px-2 py-0.5 rounded-md border border-slate-700 font-bold">
              {activeTab === 'customer' ? 'Customer Special' : 'Worker Special'}
            </span>
          </div>
        )}
        ...
        <div className="p-5 sm:p-6 overflow-y-auto space-y-5 flex-1 custom-scrollbar">
          ...
          {showActivatedSuccess ? (
            ...
            /* Customer Gold Membership Spotlight */
            <div className="space-y-4 animate-in fade-in">
              ...
              <div className="bg-gradient-to-br from-amber-500/20 via-amber-400/10 to-slate-900 border-2 border-amber-400/80 rounded-3xl p-5 relative overflow-hidden">
                <div className="flex items-start justify-between">
                  <div className="space-y-1">
                    <span className="text-[10px] font-black uppercase tracking-wider bg-amber-400 text-slate-950 px-2 py-0.5 rounded-md inline-block">
                      ⭐ YouTube-Style Channel Membership
                    </span>
                    <h3 className="text-2xl font-black text-amber-300 leading-tight">
                      Dihadi Gold Club
                    </h3>
                    <p className="text-xs text-slate-300">
                      All-inclusive 1 Month Free Service with Automated Admin Payouts
                    </p>
                  </div>
                  <div className="text-right">
                    <div className="text-3xl font-black text-white font-mono">₹15,000</div>
                    <span className="text-[10px] text-amber-400 font-bold block">1 Month Free Plan</span>
                  </div>
                </div>
                ...
                <div className="mt-4 pt-3 border-t border-amber-400/30 bg-amber-950/40 p-3 rounded-2xl border border-amber-500/30">
                  <div className="flex items-center gap-2 text-xs text-amber-200 font-bold">
                    ...
                    <span>Admin Treasury Auto-Disburses Worker Wages Directly to Worker Wallets!</span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2 text-xs">
                <div className="flex items-start gap-2.5 bg-slate-800/80 p-3 rounded-2xl border border-slate-700">
                  ...
                  <div>
                    ...
                    <p className="text-slate-400 text-[11px]">
                      When you hire or book a worker, the platform Admin directly transfers the worker's earnings straight into their wallet automatically.
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-2.5 bg-slate-800/80 p-3 rounded-2xl border border-slate-700">
                  ...
                  <div>
                    <strong className="text-white">1 Month 100% Free Booking (0% Surcharge):</strong>
                    <p className="text-slate-400 text-[11px]">
                      Post unlimited jobs with zero platform commission fees for 30 full days.
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-2.5 bg-slate-800/80 p-3 rounded-2xl border border-slate-700">
                  ...
                  <div>
                    ...
                    <p className="text-slate-400 text-[11px]">
                      Instant access to official verified worker performance dossiers and police verification badges.
                    </p>
                  </div>
                </div>
              </div>
              ...
              >
                <Crown className="w-4 h-4" />
                <span>{isProcessing ? 'Processing ₹15,000 Plan...' : 'Join Gold Club (₹15,000 / Month)'}</span>
            ...
            <div className="space-y-4 animate-in fade-in">
              ...
              <div className="bg-gradient-to-br from-amber-500/20 via-amber-400/10 to-slate-900 border-2 border-amber-400/80 rounded-3xl p-5 relative overflow-hidden">
                <div className="flex items-start justify-between">
                  <div className="space-y-1">
                    ...
                    <p className="text-xs text-slate-300">
                      Instant Govt Aadhaar Verification + 6 Zero-Commission Jobs
                    </p>
                  </div>
                  <div className="text-right">
              ...
              <div className="space-y-2 text-xs">
                ...
                <div className="flex items-start gap-2.5 bg-slate-800/80 p-3 rounded-2xl border border-slate-700">
                  <Zap className="w-4 h-4 text-amber-400 shrink-0 mt-0.5" />
                  <div>
                    <strong className="text-white">0% Platform Commission (Keep 100%):</strong>
                    <p className="text-slate-400 text-[11px]">
                      The 20% platform cut is waived for 6 jobs. If the daily wage is ₹900, you keep the full ₹900 in your wallet.
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-2.5 bg-slate-800/80 p-3 rounded-2xl border border-slate-700">
                  ...
                  <div>
                    <strong className="text-white">Top 5 Radar Priority Ranking:</strong>
                    <p className="text-slate-400 text-[11px]">
                      Your profile appears at the top of customer search and direct auto-hiring rankings.
                    </p>
                  </div>
                </div>
              </div>
        ...
        <div className="bg-slate-950 px-4 py-3 border-t border-slate-800 flex items-center justify-between text-[11px] text-slate-400 shrink-0">
          <div className="flex items-center gap-1.5 text-amber-400 font-bold">
            ...
            <span>YouTube-Style Ad Promotion</span>
          </div>
          <button
            onClick={onClose}
