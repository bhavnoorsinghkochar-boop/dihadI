...
export const GpsRadarModal: React.FC<GpsRadarModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/75 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4 animate-in fade-in duration-150 select-none">
      <div className="bg-slate-900 text-slate-100 rounded-3xl max-w-lg w-full overflow-hidden shadow-2xl border border-slate-700 flex flex-col max-h-[92vh]">
        {/* Header */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                {isWorkerPerspective ? `Navigating to job site: ${jobTitle}` : `Tracking ${workerName} (${workerTrade})`}
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-4 sm:p-5 overflow-y-auto space-y-4 flex-1 text-xs">
          ...
          <div className="bg-gradient-to-r from-amber-950/80 to-slate-800 border border-amber-500/30 rounded-2xl p-4 flex items-center justify-between shadow-inner">
            ...
            {/* Travel Mode Toggle */}
            <div className="flex flex-col gap-1 bg-slate-900/90 p-1.5 rounded-xl border border-slate-700">
              <button
                onClick={() => {
                  setTransitMode('walk');
                  playSound('click');
          ...
          {/* View Mode Tabs (Google Maps vs. Radar View) */}
          <div className="flex items-center justify-between bg-slate-800/90 p-1 rounded-xl border border-slate-700">
            <button
              onClick={() => {
          ...
          {/* Interactive Google Maps Live Frame or Radar Display */}
          {viewMode === 'google_maps' ? (
            <div className="relative w-full h-64 bg-slate-950 rounded-2xl border border-slate-700 overflow-hidden shadow-lg">
              <iframe
              ...
              <div className="absolute top-2 left-2 bg-slate-900/90 backdrop-blur-xs border border-slate-700 px-2.5 py-1.5 rounded-xl text-[10px] font-mono text-slate-300 space-y-0.5 pointer-events-none">
                ...
                <div>Origin: {liveCoords.lat.toFixed(4)}, {liveCoords.lng.toFixed(4)}</div>
                <div>Dest: {destCoords.lat.toFixed(4)}, {destCoords.lng.toFixed(4)}</div>
              </div>
            ...
            <div className="relative w-full h-64 bg-slate-950 rounded-2xl border border-slate-700 overflow-hidden shadow-lg flex items-center justify-center">
              {/* Grid Lines */}
              ...
              {/* Destination Job Pin (Employer Site) */}
              <div 
                className="absolute top-[25%] left-[80%] -translate-x-1/2 -translate-y-1/2 flex flex-col items-center group cursor-pointer z-10"
                title={jobAddress}
              >
              ...
              >
                <div className="relative">
                  <div className="w-8 h-8 rounded-full bg-amber-500 border-2 border-white shadow-xl flex items-center justify-center text-slate-950 font-black">
                    <Navigation 
                      className="w-4 h-4 transition-transform duration-300 text-slate-900"
          ...
          <div className="bg-slate-800/80 border border-slate-700 rounded-2xl p-3.5 flex items-start gap-3">
            ...
            <div className="flex-1 space-y-0.5">
              <div className="text-xs font-bold text-white">
                {currentDist <= 0.05 
                  ? '🎯 You have arrived at the job site location!' 
                  : `Navigate towards ${jobAddress.slice(0, 36)}...`}
              </div>
              <p className="text-[11px] text-slate-400">
                {currentDist <= 0.05 
                  ? 'Worker and employer are within 5 meters. Share OTP to start work timer.' 
                  : `Route synced via Google Maps • ${(currentDist).toFixed(1)} km remaining (${activeEta} mins).`}
              </p>
            </div>
          </div>

          {/* Simulation & Google Maps Action buttons */}
