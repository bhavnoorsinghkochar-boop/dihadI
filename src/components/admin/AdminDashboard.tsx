...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ isEmbedded = false }) => {
  ...
  if (!currentAdmin) {
    return (
      <div className={`bg-slate-900 text-slate-100 flex flex-col h-full overflow-y-auto select-none ${isEmbedded ? 'w-full rounded-3xl border border-slate-700' : 'max-w-md mx-auto rounded-3xl border border-slate-700 shadow-2xl'}`}>
  ...
  // IF LOGGED IN: Admin Dashboard Calculations
  const activeWorkersCount = workers.filter((w) => w.isOnline).length;
  ...
  return (
    <div className={`bg-slate-900 text-slate-100 flex flex-col h-full overflow-hidden select-none ${isEmbedded ? 'w-full rounded-3xl border border-slate-700' : 'max-w-2xl mx-auto rounded-3xl border border-slate-700 shadow-2xl'}`}>
      {/* Header */}
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  title="Simulate a new worker registering with Aadhaar"
                >
              ...
              <div className="flex bg-slate-800 p-1 rounded-xl border border-slate-700 text-[11px] font-bold">
                ...
                >
                  <span>{getT(currentLanguage, 'admin_tab_pending_kyc')}</span>
                  <span className="text-[10px]">({pendingVerifications.length})</span>
                </button>
                ...
                >
                  <span>{getT(currentLanguage, 'admin_tab_verified_kyc')}</span>
                  <span className="text-[10px]">({approvedVerifications.length})</span>
                </button>
                ...
                >
                  <span>All ({verifications.length})</span>
                </button>
              </div>
            </div>
            ...
            <div className="space-y-2.5">
              {filteredVerifications.length === 0 ? (
                ...
                filteredVerifications.map((req) => (
                  <div key={req.id} className="bg-slate-800 border border-slate-700 rounded-xl p-3.5 space-y-2.5 shadow-sm">
            ...
            <div className="space-y-2">
              {filteredWorkers.length === 0 ? (
                ...
                filteredWorkers.map((w) => (
                  <div key={w.id} className="bg-slate-800 border border-slate-700 rounded-xl p-3 flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[11px] text-slate-400 mt-0.5">
                          {w.primaryTrade} • {w.phone} • {w.location.area} • ₹{w.dailyRate}/day
                        </p>
                      </div>
                    </div>
        ...
        {adminTab === 'jobs' && (
          <div className="bg-slate-800/50 border border-slate-700 rounded-2xl p-4 space-y-2.5">
            ...
            <div className="space-y-2">
              {jobs.length === 0 ? (
                <div className="p-6 text-center text-slate-400 bg-slate-800 rounded-xl">
                  No active jobs currently in database.
                </div>
              ) : (
                jobs.map((job) => (
                  <div key={job.id} className="bg-slate-800 p-3 rounded-xl border border-slate-700 space-y-2">
                    <div className="flex justify-between items-start">
                      <div>
                        <p className="font-bold text-white text-sm">{job.title}</p>
                        <p className="text-[11px] text-slate-400">
                          {job.trade} • {job.area} • Employer: {job.customerName} ({job.customerPhone})
                        </p>
                      </div>
                    ...
                    <div className="flex items-center justify-between text-[11px] bg-slate-900/80 p-2 rounded-lg border border-slate-800 text-slate-300">
                      <span>Assigned: <strong className="text-amber-400">{job.assignedWorkerName || 'Broadcasting'}</strong></span>
                      <span>Worker Payout: <strong>₹{job.workerPayout}</strong> | Commission: <strong className="text-amber-400">₹{job.platformFee}</strong></span>
                    </div>
                  </div>
                ))
              )}
