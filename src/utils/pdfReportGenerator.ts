 ...
 */
export async function generateWorkerPerformancePdf(data: PerformanceReportData): Promise<void> {
  const {
    worker,
    completedJobs = [],
    totalEarnings,
  ...
  const cleanWorkerId = (worker.id || 'WKR-1001').replace(/[^a-zA-Z0-9]/g, '').slice(0, 8).toUpperCase();
  ...
  doc.text('OFFICIAL VERIFIED WORKER PERFORMANCE DOSSIER • CLIENT INTERVIEW REPORT', margin + 6, 25);

  doc.setFontSize(7.5);
  doc.setTextColor(148, 163, 184); // Slate-400
  doc.text(`Dossier No: ${dossierNumber}  |  Generated: ${issueDate}  |  Platform Security Tier: 1 (Encrypted)`, margin + 6, 33);
  ...
  doc.text(`${worker.primaryTrade || 'General Labour'} Specialist`, margin + 6, currentY + 16);
  ...
  doc.text(`Daily Agreed Rate: Rs. ${worker.dailyRate || 850}/day (8 hrs standard shift)`, margin + 6, currentY + 28);
  doc.text(`Experience: ${worker.experienceYears || 1}+ Years  |  Mobile: ${worker.phone || 'Verified'}`, margin + 6, currentY + 34);
  ...
  doc.setFontSize(7.5);
  doc.setFont('helvetica', 'normal');
  doc.setTextColor(worker.isVerified ? 5 : 180, worker.isVerified ? 150 : 83, worker.isVerified ? 105 : 9);
  doc.text(worker.isVerified ? '• Aadhaar KYC: VERIFIED' : '• Aadhaar KYC: SUBMITTED', pageWidth - margin - 61, currentY + 16);
  doc.setTextColor(5, 150, 105);
  doc.text('• OTP Doorstep Check: 100%', pageWidth - margin - 61, currentY + 22);
  doc.setTextColor(30, 41, 59);
  doc.text(worker.upiId ? `• UPI: ${worker.upiId.slice(0, 14)}...` : '• Direct UPI: ACTIVE', pageWidth - margin - 61, currentY + 28);
  ...
  doc.setFontSize(9.5);
  doc.setTextColor(15, 23, 42);
  doc.text('VERIFIED PERFORMANCE METRICS (AUDITED BY OTP WORK RECORD)', margin, currentY);
  ...
  doc.setFontSize(12);
  doc.setTextColor(15, 23, 42);
  const ratingText = realRating > 0 ? `${realRating.toFixed(1)} / 5.0 ★` : 'New Professional';
  doc.text(ratingText, margin + 3, currentY + 14);
  doc.setFontSize(6.5);
  doc.setFont('helvetica', 'normal');
  doc.setTextColor(180, 83, 9);
  const reviewSubtext = totalReviewsCount > 0 ? `${totalReviewsCount} Verified Review${totalReviewsCount > 1 ? 's' : ''}` : '0 Client Reviews';
  ...
  doc.setFontSize(13);
  doc.setTextColor(15, 23, 42);
  doc.text(`${completedCount} Job${completedCount === 1 ? '' : 's'}`, col2X + 3, currentY + 14);
  doc.setFontSize(6.5);
  doc.setFont('helvetica', 'normal');
  doc.setTextColor(4, 120, 87);
  doc.text(completedCount > 0 ? '100% Payout & Escrow Done' : 'Ready for Hiring', col2X + 3, currentY + 20);
  ...
  doc.setFontSize(13);
  doc.setTextColor(15, 23, 42);
  doc.text(completedCount > 0 ? '100%' : '100% Ready', col3X + 3, currentY + 14);
  doc.setFontSize(6.5);
  doc.setFont('helvetica', 'normal');
  ...
  doc.text(`Equipment Checklist: Professional tools, helmet, precision level meter, measuring tape & safety boots.`, margin + 4, currentY + 17);
  ...
  doc.setFontSize(9.5);
  doc.setTextColor(15, 23, 42);
  doc.text(`VERIFIED EMPLOYER ASSIGNMENT LOG (${completedJobs.length} COMPLETED RECORDS)`, margin, currentY);
  ...
  if (completedJobs.length === 0) {
    ...
    doc.setFontSize(8.5);
    doc.text('No completed assignments recorded on platform yet.', margin + 6, currentY + 10);
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(7.5);
    doc.setTextColor(148, 163, 184);
    doc.text('Worker is verified, registered with active daily wage status, and available for immediate hiring.', margin + 6, currentY + 17);
    currentY += 28;
  } else {
    ...
    jobsToDisplay.forEach((j: any, index: number) => {
      ...
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(8);
      doc.text((j.title || `${j.trade || worker.primaryTrade} Assignment`).slice(0, 36), margin + 3, currentY + 5);

      doc.setFont('helvetica', 'normal');
      doc.setFontSize(6.8);
      doc.setTextColor(100, 116, 139);
      doc.text((j.locationAddress || j.area || `${worker.location?.area || 'Ludhiana'}, Punjab`).slice(0, 38), margin + 3, currentY + 10);

      doc.setFont('helvetica', 'bold');
      doc.setTextColor(30, 41, 59);
      doc.text((j.customerName || 'Verified Employer').slice(0, 26), margin + 70, currentY + 5);
      doc.setFont('helvetica', 'italic');
      doc.setFontSize(6.5);
      doc.setTextColor(71, 85, 105);
      const reviewText = j.review || j.customerReview || j.reviewGiven || 'Task finished and verified.';
      ...
      doc.setFontSize(6.5);
      doc.setFont('helvetica', 'bold');
      doc.text(`✓ OTP #${j.otpCode || 'VERIFIED'}`, margin + 148, currentY + 10);
  ...
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(8.5);
  doc.text('OFFICIAL PLATFORM CERTIFICATION & SIGNATURE', margin + 6, currentY + 8);

  doc.setFont('helvetica', 'normal');
  doc.setFontSize(7);
  doc.setTextColor(71, 85, 105);
  doc.text('This document certifies that the performance, rating, daily wage, and attendance history', margin + 6, currentY + 14);
  doc.text('of this worker are digitally cryptographically logged via Dihadi Escrow and Start-OTP protocols.', margin + 6, currentY + 19);
  doc.text('Valid for Client Interviews, Contractor Tender Onboarding, and Skill Verification.', margin + 6, currentY + 24);
  ...
  doc.setTextColor(100, 116, 139);
  doc.text('Authorized Labour Compliance', pageWidth - margin - 54, currentY + 21);
  doc.text(`ID: #DIHADI-AUTH-${cleanWorkerId}`, pageWidth - margin - 54, currentY + 26);

  // Bottom Footer
  doc.setFontSize(7);
  doc.setTextColor(148, 163, 184);
  doc.text(`Dihadi Platform • India's Verified Skilled Daily-Wage Labour Grid • Support: help@dihadi.in • Page 1 of 1`, margin, pageHeight - 6);
