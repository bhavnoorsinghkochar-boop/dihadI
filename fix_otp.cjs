...
const regex = /{job\.escrowStatus === 'pending' \? \([\s\S]*?\(job\.status === 'accepted' \|\| job\.status === 'broadcast' \|\| job\.status === 'in_progress'\) && \(\{[\s\S]*?\&\& \(\ \(/g;

const replacement = `{job.escrowStatus === 'pending' ? (
                    <div className="bg-amber-100 p-4 rounded-2xl border border-amber-300 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                      ...
                      <button type="button" onClick={() => setPrepayBooking({ type: 'approve_escrow', jobId: job.id, amount: job.dailyWage * job.durationDays, workerName: job.assignedWorkerName || 'Worker' })} className="px-5 py-2.5 bg-amber-500 hover:bg-amber-400 text-slate-950 font-black rounded-xl text-xs transition shadow-sm whitespace-nowrap">
