...
import { compressImageFile, WORKER_AVATAR_PRESETS } from '../../utils/imageUtils';
...
export const WorkerAvatarUploadModal: React.FC<WorkerAvatarUploadModalProps> = ({
  isOpen,
  onClose,
  worker,
  onAvatarUpdated,
  ...
  const [selectedAvatar, setSelectedAvatar] = useState<string | null>(worker?.avatar || null);
  ...
  const currentWorkerAvatar = selectedAvatar || worker?.avatar || 'https://images.unsplash.com/photo-1541888946425-d0fbb18086f6?w=200&auto=format&fit=crop&q=80';
  ...
  const handleFileProcess = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      // Compress to optimal profile avatar size (500x500px, 85% quality)
      ...
      setError(err?.message || 'Failed to process selected photo. Please try a different image.');
      playSound('alert');
    } finally {
  ...
  const handleSaveAvatar = () => {
    ...
    setTimeout(() => {
      ...
      if (showNotification) {
        showNotification('Profile Photo Updated', 'Your verified profile picture is now live.');
      }
        ...
        <div className="bg-slate-900 text-white p-5 relative shrink-0">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-base sm:text-lg font-black text-white">Upload Worker Photo</h3>
                <p className="text-[11px] text-slate-400">
                  A clear profile photo increases daily hiring requests by 85%
                </p>
              </div>
            </div>
        ...
        <div className="p-5 space-y-5 text-slate-800 text-xs overflow-y-auto max-h-[75vh]">
          ...
          <div className="flex flex-col items-center justify-center p-4 bg-slate-50 border border-slate-200 rounded-2xl space-y-3">
              ...
              <div className="w-28 h-28 rounded-full overflow-hidden border-4 border-amber-400 shadow-xl bg-slate-800 flex items-center justify-center">
                <img
                  ...
                  onError={(e) => {
                    ...
                    (e.target as HTMLImageElement).src = 'https://images.unsplash.com/photo-1541888946425-d0fbb18086f6?w=200&auto=format&fit=crop&q=80';
                  }}
            ...
            <div className="text-center">
              <h4 className="font-bold text-sm text-slate-900">{worker?.name || 'Professional Worker'}</h4>
              <p className="text-[11px] text-slate-500 font-semibold">{worker?.primaryTrade || 'Artisan Craftsman'}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2.5">
            ...
            >
              ...
              <span className="font-bold text-amber-950 text-xs">Take Photo</span>
              <span className="text-[10px] text-slate-500">Instant camera capture</span>
            </button>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <label className="font-black text-slate-700 uppercase tracking-wider text-[11px] flex items-center gap-1">
                ...
                <span>Or Choose Professional Avatar</span>
              </label>
              ...
              {WORKER_AVATAR_PRESETS.map((preset) => {
                ...
                return (
                  <button
                    ...
                    className={`relative rounded-2xl overflow-hidden border-2 transition p-0.5 aspect-square cursor-pointer group ${
                      isSelected
