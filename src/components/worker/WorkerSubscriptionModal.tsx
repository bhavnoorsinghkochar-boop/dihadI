...
interface WorkerSubscriptionModalProps {
  ...
  onSubscribe: (method: 'WALLET' | 'UPI') => Promise<{ success: boolean; message: string }> | { success: boolean; message: string };
  onTopUpWallet?: (amount: number) => void;
}

export const WorkerSubscriptionModal: React.FC<WorkerSubscriptionModalProps> = ({
  isOpen,
  onClose,
  worker,
  onSubscribe,
  ...
  const handleActivate = async () => {
    ...
    try {
      if (selectedMethod === 'WALLET') {
        ...
        // UPI Flow: Show QR or instant confirm
        setShowUpiQr(true);
      }
    } catch (e) {
      console.error(e);
    ...
    <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-3 sm:p-4 animate-in fade-in duration-200">
      <div className="bg-gradient-to-b from-slate-900 via-slate-900 to-slate-950 border border-amber-500/40 rounded-3xl max-w-lg w-full overflow-hidden shadow-2xl text-white relative max-h-[92vh] flex flex-col">
          ...
          {showSuccessState ? (
            <div className="text-center py-6 space-y-4 animate-in zoom-in-95">
              ...
              <div className="space-y-1">
                <h3 className="text-xl font-black text-white">VIP Pass Activated Successfully!</h3>
                <p className="text-xs text-amber-300 font-medium">
                  {selectedMethod === 'WALLET' ? '₹2,000 directly deducted from your wallet balance.' : '₹2,000 paid via UPI.'}
                </p>
                <p className="text-xs text-slate-300 pt-2">
                  You have <strong className="text-amber-400 font-bold">{remainingZeroJobs + JOBS_COUNT} Zero-Commission Jobs</strong> ready! You will keep 100% of your earnings on every job.
                </p>
              </div>

              <div className="bg-slate-800/80 border border-slate-700 rounded-2xl p-4 text-left space-y-2">
                <div className="flex items-center justify-between text-xs">
                  <span className="text-slate-400">Zero-Fee Jobs Credit:</span>
                  <span className="font-black text-amber-400 font-mono">+6 Jobs</span>
                </div>
                <div className="flex items-center justify-between text-xs">
                ...
                <div className="flex items-center justify-between text-xs border-t border-slate-700/80 pt-2">
                  ...
                  <span className="font-black text-white font-mono">₹{worker.walletBalance}</span>
                </div>
              </div>
              ...
              >
                Start Taking Zero-Commission Jobs
              </button>
            </div>
          ) : showUpiQr ? (
            /* UPI QR Code Screen */
            <div className="space-y-4 text-center animate-in fade-in">
              <div className="space-y-1">
                <h3 className="text-lg font-black text-white">Scan UPI QR to Pay ₹{PRICE}</h3>
              ...
              <div className="bg-white p-4 rounded-2xl max-w-[200px] mx-auto shadow-xl border-2 border-amber-400 flex flex-col items-center">
                <img
                  src={`https://api.qrserver.com/v1/create-qr-code/?size=180x180&data=${encodeURIComponent(
                    `upi://pay?pa=dihadi.vip@okhdfcbank&pn=Dihadi%20VIP%20Pass&am=${PRICE}&cu=INR&tn=Worker%20VIP%206%20Jobs%20Pass`
                  )}`}
                  alt="UPI QR"
                  className="w-36 h-36 rounded-lg"
                />
                <span className="text-[10px] text-slate-600 font-mono font-bold mt-2">
                  dihadi.vip@okhdfcbank
                </span>
              </div>
              ...
              <div className="bg-slate-800/80 border border-slate-700 rounded-2xl p-3 text-xs text-left space-y-1.5">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-bold text-amber-300">6 Zero-Commission Jobs Pass</span>
                </div>
            ...
            <>
              ...
              <div className="bg-gradient-to-br from-slate-800 to-slate-850 border border-slate-700 rounded-3xl p-4 sm:p-5 relative overflow-hidden">
                <div className="flex items-baseline justify-between">
                  <div>
                    <span className="text-[10px] font-bold text-amber-400 uppercase tracking-wider block">
                      Special Worker Package
                    </span>
                    <div className="flex items-baseline gap-1.5 mt-0.5">
                      <span className="text-3xl font-black text-white font-mono">₹{PRICE}</span>
                      <span className="text-xs text-slate-400 font-medium">one-time pass</span>
                    </div>
                  </div>
              ...
              <div className="space-y-2.5">
                ...
                <div className="grid grid-cols-1 gap-2 text-xs">
                  ...
                  <div className="flex items-start gap-2.5 bg-slate-850 p-2.5 rounded-xl border border-slate-800">
                    ...
                    <div>
                      <strong className="text-white">6 Jobs Without Commission:</strong>
                      <p className="text-slate-400 text-[11px]">
                        The 20% platform cut is waived to 0%. If wage is ₹900, you get full ₹900 directly into your wallet!
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-2.5 bg-slate-850 p-2.5 rounded-xl border border-slate-800">
                    ...
                    <div>
                      ...
                      <p className="text-slate-400 text-[11px]">
                        The pass easily pays for itself within the first few jobs.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-2.5 bg-slate-850 p-2.5 rounded-xl border border-slate-800">
                    ...
                    <div>
                      <strong className="text-white">Top 5 Radar Priority:</strong>
                      <p className="text-slate-400 text-[11px]">
                        VIP badge workers appear with top priority ranking in client search results and auto-hiring.
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="space-y-3 pt-1">
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className={`w-9 h-9 rounded-xl flex items-center justify-center font-bold ${
                      selectedMethod === 'WALLET' ? 'bg-amber-500 text-slate-950' : 'bg-slate-800 text-slate-400'
                    }`}>
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-slate-400">
                        Current Wallet: <strong className="text-amber-400 font-mono">₹{walletBalance}</strong>
                      </p>
                    </div>
                  </div>

                  <div className="text-right">
                    <span className="text-xs font-mono font-black">₹{PRICE}</span>
                  </div>
                </div>
                ...
                {selectedMethod === 'WALLET' && !hasEnoughWalletBalance && (
                  <div className="bg-rose-500/10 border border-rose-500/30 rounded-2xl p-3 text-xs space-y-2 animate-in fade-in">
                    ...
                    <p className="text-[11px] text-slate-300">
                      You need ₹{PRICE - walletBalance} more in your wallet to activate directly.
                    </p>
                    <div className="flex gap-2">
                      <button
                      ...
                      >
                        + Add ₹{PRICE - walletBalance} to Wallet
                      </button>
                      <button
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className={`w-9 h-9 rounded-xl flex items-center justify-center font-bold ${
                      selectedMethod === 'UPI' ? 'bg-amber-500 text-slate-950' : 'bg-slate-800 text-slate-400'
                    }`}>
                      <QrCode className="w-5 h-5" />
                    </div>
                    <div>
                      <span className="text-xs font-black">Pay via UPI QR / GPay / PhonePe</span>
                      <p className="text-[11px] text-slate-400">
              ...
              <div className="pt-2">
                <button
                  ...
                  disabled={isProcessing || (selectedMethod === 'WALLET' && !hasEnoughWalletBalance)}
        ...
        <div className="bg-slate-950 p-3 text-center border-t border-slate-800/80 text-[10px] text-slate-500 shrink-0">
          🔒 Transparent 0% Commission Policy • Instant automatic activation • No hidden charges
        </div>
      </div>
    </div>
  );
