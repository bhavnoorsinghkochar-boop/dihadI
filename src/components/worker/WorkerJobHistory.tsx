...
export const WorkerJobHistory: React.FC<WorkerJobHistoryProps> = ({
  ...
  const [sortBy, setSortBy] = useState<'newest' | 'highest_payout' | 'highest_rating'>('newest');
  ...
  // Strictly use actual completed assignments
  ...
  const filteredJobs = useMemo(() => {
    return displayJobs
      .filter((j) => {
        // 1. Trade filter
        if (selectedTrade !== 'All' && j.trade !== selectedTrade) {
          return false;
        }
        // 2. Rating filter
        if (selectedRatingFilter === '5_stars' && ((j.rating || j.customerRating || 0) < 5)) {
          return false;
        }
        if (selectedRatingFilter === '4_plus' && ((j.rating || j.customerRating || 0) < 4)) {
          return false;
        }
        // 3. Search query
        if (searchQuery.trim()) {
          ...
          const matchArea = (j.area || j.locationAddress || '').toLowerCase().includes(q);
          const matchTxn = (j.transactionRef || '').toLowerCase().includes(q);
          if (!matchTitle && !matchDesc && !matchCust && !matchArea && !matchTxn) {
            return false;
          }
        }
        return true;
      ...
      .sort((a, b) => {
        if (sortBy === 'highest_payout') {
          return (b.workerPayout || b.dailyWage) - (a.workerPayout || a.dailyWage);
        }
        if (sortBy === 'highest_rating') {
          return (b.rating || b.customerRating || 0) - (a.rating || a.customerRating || 0);
        }
        // newest first (default)
        return (new Date(b.ratedAt || b.completedAt || b.postedAt).getTime() || 0) - 
  ...
  const ratedList = useMemo(() => {
    return displayJobs.filter((j) => (j.rating && j.rating > 0) || (j.customerRating && j.customerRating > 0) || (j.ratingGiven && j.ratingGiven > 0));
  }, [displayJobs]);
  ...
  const avgRating = useMemo(() => {
    ...
    const sum = ratedList.reduce((acc, j) => acc + (j.rating || j.customerRating || j.ratingGiven || 0), 0);
      ...
      <div className="bg-slate-900 text-white rounded-3xl p-6 sm:p-7 border border-slate-800 shadow-xl space-y-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-xl font-black text-white">Certified Job & Payout History</h3>
            </div>
            <p className="text-xs text-slate-400">
              Verified record of completed daily wage assignments, client reviews, and direct UPI settlements.
            </p>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              title="Download official verified performance report as PDF for client interviews"
            >
              {isExportingPdf ? (
                <>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-2 border-t border-slate-800">
          ...
          <div className="bg-slate-800/80 p-3.5 rounded-2xl border border-slate-700/60 space-y-1">
            ...
            <p className="text-2xl font-black text-amber-400 font-mono">₹{totalEarned.toLocaleString('en-IN')}</p>
          ...
          <div className="bg-slate-800/80 p-3.5 rounded-2xl border border-slate-700/60 space-y-1">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Client Rating</span>
            ...
            <span className="text-[10px] text-amber-300 font-medium">
              {ratedList.length > 0 ? `${ratedList.length} Verified Review${ratedList.length > 1 ? 's' : ''}` : 'No Ratings Yet'}
            </span>
          </div>
          ...
          <div className="bg-slate-800/80 p-3.5 rounded-2xl border border-slate-700/60 space-y-1">
            ...
            <span className="text-[10px] text-slate-400 font-medium">
              {displayJobs.length > 0 ? 'Strict 10km GPS Radar' : 'No jobs yet'}
            </span>
          </div>
        </div>
      </div>
      ...
      {displayJobs.length === 0 ? (
        <div className="bg-white border border-slate-200 rounded-3xl p-8 sm:p-12 text-center space-y-4 shadow-xs max-w-xl mx-auto">
          ...
          <div className="space-y-1.5">
            <h4 className="text-lg font-black text-slate-900">No Completed Jobs Recorded Yet</h4>
            <p className="text-xs text-slate-500 max-w-md mx-auto leading-relaxed">
              When you accept daily job broadcasts, verify start OTP with employers, and complete the work, your verified completion receipts, instant UPI earnings, and employer testimonials will show here automatically.
            </p>
          </div>
          <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200 text-left text-xs space-y-2">
            <p className="font-bold text-slate-800 flex items-center gap-1.5">
              <Sparkles className="w-3.5 h-3.5 text-amber-500" />
              <span>How your performance builds:</span>
            </p>
            <ul className="text-slate-600 space-y-1 text-[11px] list-disc list-inside">
              <li>Accept broadcast jobs in your trade from the <strong>Available Jobs</strong> tab.</li>
              <li>Provide your 4-digit start OTP when you reach the customer's site.</li>
              <li>Customer releases payment directly to your UPI ID (<strong className="text-slate-900">{worker.upiId}</strong>) upon completion.</li>
            </ul>
          </div>
        </div>
      ) : (
          ...
          <div className="bg-white border border-slate-200 rounded-3xl p-4 sm:p-5 shadow-xs space-y-4">
            <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3">
              ...
              <div className="relative flex-1">
                ...
                <input
                  type="text"
                  placeholder="Search past jobs by title, employer, area, or UPI transaction ref..."
                  value={searchQuery}
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                >
                  ...
                  <option value="highest_payout">💰 Highest Net Payout</option>
                  <option value="highest_rating">⭐ Highest Client Rating</option>
                </select>
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2 pt-1 border-t border-slate-100">
              ...
              {['All', 'Mason', 'Painter', 'Plumber', 'Carpenter', 'Electrician', 'Labour'].map((trade) => (
                <button
                  key={trade}
          ...
          <div className="space-y-4">
            ...
            {filteredJobs.length === 0 ? (
              <div className="bg-white border border-slate-200 rounded-3xl p-12 text-center space-y-3 shadow-sm max-w-md mx-auto">
                ...
                <h4 className="text-base font-black text-slate-900">No Past Jobs Match Filters</h4>
                <p className="text-xs text-slate-500">
                  Try changing your search query or trade filters to see other past completed jobs.
                </p>
                <button
                  onClick={() => { setSearchQuery(''); setSelectedTrade('All'); setSelectedRatingFilter('All'); }}
                ...
                {filteredJobs.map((job) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {(job.rating || job.customerRating || job.review || job.customerReview || job.reviewGiven) && (
                        <div className="bg-amber-50/70 border border-amber-200/80 rounded-2xl p-4 space-y-2.5">
                          <div className="flex items-center justify-between gap-2">
                            ...
                            <button
                              ...
                              title="Listen to employer review"
                            >
                              <Volume2 className="w-3.5 h-3.5 text-amber-700" />
                              <span className="hidden sm:inline">Listen Review</span>
                            </button>
                          </div>
                      ...
                      <div className="pt-2 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 text-xs">
                        <div className="flex items-center gap-2 font-mono text-[11px] text-slate-500">
                          ...
                          <span>Ref: <strong className="text-slate-800">{txn}</strong></span>
                          <button
                            type="button"
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            title="View conversation with employer"
                          >
                            <MessageSquare className="w-3.5 h-3.5 text-slate-600" />
                            <span>Employer Chat</span>
                          </button>
          ...
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 space-y-6 shadow-2xl border border-slate-200 my-auto text-slate-900">
            ...
            <div className="flex justify-between items-start border-b border-slate-200 pb-4">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <h4 className="font-black text-base tracking-tight">DIHADI OFFICIAL SETTLEMENT RECEIPT</h4>
                </div>
                <p className="text-[11px] text-slate-500 mt-0.5">Govt. Verified Hyperlocal Escrow Settlement</p>
              </div>
              <button
            ...
            <div className="space-y-4 text-xs">
              ...
              <div className="grid grid-cols-2 gap-3 p-3.5 bg-slate-50 rounded-2xl border border-slate-200/80 text-[11px]">
                <div>
                  <span className="text-slate-400 font-medium block">Worker Name</span>
                  <span className="font-bold text-slate-800">{worker.name}</span>
                </div>
                <div>
                  <span className="text-slate-400 font-medium block">Trade</span>
                  <span className="font-bold text-slate-800">{receiptJob.trade}</span>
                </div>
                <div>
                  <span className="text-slate-400 font-medium block">Employer</span>
                  <span className="font-bold text-slate-800">{receiptJob.customerName}</span>
                </div>
                <div>
                  <span className="text-slate-400 font-medium block">Job Location</span>
                  <span className="font-bold text-slate-800 truncate block">{receiptJob.area}</span>
                </div>
                <div>
                  <span className="text-slate-400 font-medium block">Payment Channel</span>
                  <span className="font-mono font-bold text-slate-800">{receiptJob.paidVia || 'UPI_QR'}</span>
                </div>
                <div>
                  <span className="text-slate-400 font-medium block">Transaction Ref</span>
                  <span className="font-mono font-bold text-slate-800 truncate block">{receiptJob.transactionRef || `UPI-DIHADI-${receiptJob.id.slice(-6).toUpperCase()}`}</span>
                </div>
              </div>
              ...
              <div className="space-y-2 border-t border-slate-200 pt-3">
                <div className="flex justify-between text-slate-600">
                  <span>Gross Job Daily Wage</span>
