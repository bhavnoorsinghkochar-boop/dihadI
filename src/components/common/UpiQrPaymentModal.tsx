...
export const UpiQrPaymentModal: React.FC<UpiQrPaymentModalProps> = ({
  ...
  const [review, setReview] = useState<string>('Great work, punctual and high quality finish!');
  const [selectedTags, setSelectedTags] = useState<string[]>(['⚡ Punctual & On-Time', '🛠️ Expert Craftsmanship']);
  ...
  const handleFinalizeRating = () => {
    ...
    if (completedTxn && onPaymentSuccess) {
      onPaymentSuccess(
        ...
        completedTxn.txnId, 
        rating, 
        review || `Rated ${rating} stars for work on ${jobTitle || 'Dihadi Job'}`,
        selectedTags
      );
    }
    onClose();
        ...
        <div className="bg-gradient-to-r from-slate-900 to-slate-800 text-white p-4 sm:p-5 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl bg-amber-500/20 border border-amber-400/30 flex items-center justify-center text-amber-400">
              {step === 'rating' ? <Star className="w-5 h-5 fill-amber-400 text-amber-400" /> : <QrCode className="w-5 h-5" />}
            </div>
            <div>
              ...
              <p className="text-xs text-slate-300 font-medium">
                {step === 'rating' ? `Rate performance for ${targetWorkerName}` : subtitle}
              </p>
            </div>
          </div>
          <button
              ...
              <div className="flex flex-col items-center justify-center p-3 bg-slate-50 rounded-2xl border border-slate-200 space-y-3">
                ...
                <div className="text-center space-y-1">
                  ...
                  <p className="text-[11px] text-slate-500">
                    NPCI interoperable standard • Direct bank credit to {targetWorkerName}
                  </p>
                </div>
          ...
          <div className="p-4 sm:p-5 overflow-y-auto space-y-4 flex-1 animate-in fade-in">
            ...
            <div className="bg-amber-50/80 border border-amber-200 rounded-2xl p-4 text-center space-y-2">
              <span className="text-xs font-black text-amber-900 uppercase tracking-wider block">
                Rate {targetWorkerName} ({workerTrade || 'Worker'})
              </span>
              ...
              <div className="h-5 flex items-center justify-center">
                <span className="text-xs font-black text-slate-900 bg-white px-3 py-0.5 rounded-full border border-amber-200">
                  {currentDisplayRating} ★ — {RATING_DESCRIPTIONS[currentDisplayRating]}
                </span>
              </div>
            </div>
        ...
        <div className="p-4 bg-slate-50 border-t border-slate-200 flex items-center justify-between gap-3 shrink-0">
          {step === 'payment' ? (
            ...
            >
              ...
              <span>Submit Rating & Complete Job</span>
            </button>
          )}
        </div>
      </div>
